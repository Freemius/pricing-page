...
function sortPricingByLicensesAsc(allPlansPricingCollection) {
  allPlansPricingCollection.sort(function (pricingA, pricingB) {
    ...
    if (
      pricingB.isUnlimited() ||
      (!pricingA.isUnlimited() && pricingA.licenses < pricingB.licenses)
    ) {
      return -1;
    }

    if (
      pricingA.isUnlimited() ||
      (!pricingB.isUnlimited() && pricingA.licenses > pricingB.licenses)
    ) {
      return 1;
    }
  });
...
function getInstance(plans) {
  ...
  _instance = {
    calculateMultiSiteDiscount: function (pricing, billingCycle) {
      ...
      if (
        !pricing.hasMonthlyPrice() ||
        BillingCycle.ANNUAL !== billingCycleInMonths
      ) {
        ...
        singleSitePrice =
          this.tryCalcSingleSitePrice(pricing, BillingCycle.ANNUAL) / 12;
