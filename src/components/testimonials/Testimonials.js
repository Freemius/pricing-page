...
class Testimonials extends Component {
  ...
  render() {
    ...
    (function () {
      setTimeout(function () {
        let carouselInterval = null,
          firstVisibleIndex = 0,
          maxVisibleReviews = 3,
          $testimonialsSection = document.querySelector(
        ...
        let slide = function (selectedIndex, isInvisible) {
          ...
          if (isInvisible) $testimonialsSection.classList.remove('ready');

          let shiftedIndex = maxVisibleReviews + selectedIndex,
            selectedBullet =
              ((selectedIndex % uniqueTestimonials) + uniqueTestimonials) %
              uniqueTestimonials;
        ...
        let adjustTestimonials = function () {
          ...
          $testimonialsContainer.style.width =
            $testimonials.length * cardWidth + 'px';
          ...
          $testimonialsContainer.style.left =
            ('left',
            -1 * ((firstVisibleIndex + maxVisibleReviews) * cardWidth) + 'px');
    ...
    for (
      let i = -maxVisibleReviews;
      i < reviewsCount + maxVisibleReviews;
      i++
    ) {
      let review =
        pricingData.reviews[((i % reviewsCount) + reviewsCount) % reviewsCount];

      let defaultPicIndex = review.email
        ? (review.email.charAt(0).toLowerCase().charCodeAt(0) -
            'a'.charCodeAt(0)) %
          5
      ...
      reviews.push(
        <section
          className={
            'fs-testimonial' + (i < 0 || i >= reviewsCount ? ' clone' : '')
          }
          data-index={i}
          data-id={review.id}
          key={i}
    ...
    return (
      <Fragment>
        {pricingData.active_installs > 1000 && (
          <header className="fs-section-header">
            <h2>
              ...
              {Helper.formatNumber(
                Math.ceil(pricingData.active_installs / 1000) * 1000
              )}{' '}
        ...
        {pricingData.active_installs <= 1000 && pricingData.downloads > 1000 && (
          <header className="fs-section-header">
            <h2>
              ...
              {Helper.formatNumber(
                Math.ceil(pricingData.downloads / 1000) * 1000
              )}{' '}
              Times!
            </h2>
          </header>
        ...
        <section className="fs-testimonials-nav">
          ...
          <nav className="fs-nav fs-nav-next">
            <Icon icon={['fas', 'arrow-right']} />
          </nav>
        </section>
        <ul className="fs-nav fs-nav-pagination slick-dots" role="tablist">
          {dots}
        </ul>
      </Fragment>
    );
