...
class Faq extends Component {
  ...
  render() {
    ...
    if (context.hasEmailSupportForAllPlans) {
      ...
      faqSupportAnswer = (
        <Fragment>
          ...
          to get help from the community. Unfortunately extra support is
    ...
    faq.push({
      ...
      a:
        `Yes, if you ever decide that ${context.plugin.title} isn't the best ${moduleLabel} for your business, simply cancel your account from your Account panel.` +
        (isBlocking
          ? ''
          : (isNonBlocking
              ? " You'll"
              : ' If you cancel ' +
                (!isBlockingAnnually ? 'an annual' : 'a monthly') +
                " subscription, you'll") +
    ...
    if (hasMonthlyCycle || hasAnnualCycle) {
      ...
      else if (hasMonthlyCycle) answer = 'All plans are month-to-month.';
      else if (hasAnnualCycle) answer = 'All plans are year-to-year.';
    ...
    if (
      hasAnnualCycle &&
      context.plugin.hasRenewalsDiscount(BillingCycle.ANNUAL)
    ) {
      faq.push({
    ...
    faq.push({
      ...
      a: context.isPayPalSupported ? (
        ...
        <Fragment>
          ...
          American Express.
          <br />
          Unfortunately, due to regulations in your country related to PayPal’s
          subscriptions, we won’t be able to accept payments via PayPal.
        </Fragment>
      ),
    });
      ...
      if (RefundPolicyType.STRICT !== context.plugin.refund_policy) {
        refundAnswer = (
          <Fragment>
            ...
            We stand behind the quality of our product and will refund 100% of
            your money if you are unhappy with the plugin.
          </Fragment>
        );
      } else {
        refundAnswer = (
          <Fragment>
            ...
            We stand behind the quality of our product and will refund 100% of
            your money if you experience an issue that makes the plugin unusable
    ...
    if (context.hasPremiumVersion) {
      faq.push({
        ...
        a:
          `Yes! Automatic updates to our premium ${moduleLabel} are available free of charge as long as you stay our paying customer.` +
          (isBlocking
            ? ''
            : ' If you cancel your ' +
              (isNonBlocking
    ...
    for (let i = 0; i < faq.length; i++) {
      faqElements.push(
        <Section key={i} fs-section="faq-item">
          <h3>{faq[i]['q']}</h3>
          <p>{faq[i]['a']}</p>
        </Section>
      );
